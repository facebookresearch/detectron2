...
class Boxes:
    ...
    # 1. staticmethod is supported by torchscript, But we can not use `staticmethod` + `type`,
    # because torchscript only supports function `type` with input type `torch.Tensor`.
    # 2. classmethod is not fully supported by torchscript. We explicitly assign cls to Box as
    # a workaround to get torchscript support.
    @classmethod
