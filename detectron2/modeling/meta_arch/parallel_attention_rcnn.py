...
__all__ = ["ParallelAttRCNN"]


@META_ARCH_REGISTRY.register()
class ParallelAttRCNN(nn.Module):
    """
    ...
    def __init__(self, cfg):
        super().__init__()
        self.cross_share = cfg.MODEL.ATTENTION_ROI_HEAD.CROSS_SHARE
        self.device = torch.device(cfg.MODEL.DEVICE)
        self.backbone = build_backbone(cfg)
        self.index_list = cate_id_list()
        self.classes_num_each_freq = [cfg.DATASETS.NUM_CLASSES_F, cfg.DATASETS.NUM_CLASSES_C, cfg.DATASETS.NUM_CLASSES_R]
    ...
    def num_class_modifier(self, cfg, mode):
        ...
        if self.cross_share:
            ...
            if mode == 'c':
                cfg.MODEL.ROI_HEADS.NUM_CLASSES = cfg.DATASETS.NUM_CLASSES_C + cfg.DATASETS.NUM_CLASSES_F
            if mode == 'r':
                cfg.MODEL.ROI_HEADS.NUM_CLASSES = cfg.DATASETS.NUM_CLASSES_R + cfg.DATASETS.NUM_CLASSES_C + cfg.DATASETS.NUM_CLASSES_F
        else:
            if mode == 'f':
                cfg.MODEL.ROI_HEADS.NUM_CLASSES = cfg.DATASETS.NUM_CLASSES_F
            if mode == 'c':
    ...
    def forward(self, batched_inputs):
        ...
        if self.cross_share:
            gt_instances_fc = gt_instances_c.copy()
            gt_fc_list = [self.Instance_shifter(gt_instances_fc, self.classes_num_each_freq[0], training = self.training), gt_instances_f]
            gt_instances_fc = self.Instance_combinator(gt_fc_list)
            gt_instances_fcr = gt_instances_r.copy()
            gt_fcr_list = [self.Instance_shifter(gt_instances_fcr, self.classes_num_each_freq[0] + self.classes_num_each_freq[1], training = self.training) ,gt_instances_fc]
            gt_instances_fcr =  self.Instance_combinator(gt_fcr_list)
...
#             print(len(proposals_f[0].get('proposal_boxes')))
#             print(len(proposals_f[0].get('objectness_logits')))
        else:
...
#         print(len(proposals_f))
#         print(proposals_f[0].get('proposal_boxes'))
#         print(proposals_f[0].get('objectness_logits'))
...
#         exit()
        ...
        if self.cross_share:
            _, detector_losses_c = self.roi_heads_c(images, features, proposals_c, gt_instances_cf)
        else:
...
#         proposal_losses_total = self.combine_loss([proposal_losses_f, proposal_losses_c, proposal_losses_r])    
    ...
    def inference(self, batched_inputs, detected_instances=None, do_postprocess=True):
        ...
        if self.cross_share:
            results_c = self.Instance_shifter(results_c, -1*self.classes_num_each_freq[0], training = self.training)
            results_r = self.Instance_shifter(results_r, -1*(self.classes_num_each_freq[0]+self.classes_num_each_freq[1]), training = self.training)
    ...
    def combine_result(self, result_list):
        ...
        for idx_batch in range(num_batchs):
            tmp_list = []
            for udx, result in enumerate(result_list):
                Inst = result[idx_batch]
                Inst.classes_reindex(self.index_list, udx)
                tmp_list.append(Inst)
            new_instance = I_type.cat(tmp_list)
            results.append(new_instance)
