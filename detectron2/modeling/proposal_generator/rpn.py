...
class StandardRPNHead(nn.Module):
    ...
    def __init__(
        ...
        if len(conv_dims) == 1:
            ...
            self.conv = self.get_rpn_conv(cur_channels, out_channels)
            ...
            for k, conv_dim in enumerate(conv_dims):
                ...
                conv = self.get_rpn_conv(cur_channels, out_channels)
                self.conv.add_module(f"rpn_conv{k}", conv)
                cur_channels = out_channels
        ...
        if len(conv_dims) == 1:
            self.rpn_layer_init(self.conv)
        else:
            for l in self.conv:
                self.rpn_layer_init(l)
        for l in [self.objectness_logits, self.anchor_deltas]:
            self.rpn_layer_init(l)

    def get_rpn_conv(self, in_channels, out_channels):
        return Conv2d(
            in_channels,
            out_channels,
            kernel_size=3,
    ...
    def rpn_layer_init(self, module):
    ...
    def from_config(cls, cfg, input_shape):
        ...
        conv_dim = cfg.MODEL.RPN.CONV_DIM
