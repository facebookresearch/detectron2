...
class TestCheckpointer(unittest.TestCase):
    ...
    def test_load_with_matching_heuristics(self):
        with tempfile.TemporaryDirectory(prefix="detectron2_test") as d:
            ...
            checkpointer.__load(os.path.join(d, "checkpoint.pth"))
            self.assertTrue(model.block1.layer1.weight.equal(torch.ones(3, 2)))

            # load checkpoint with matching_heuristics
            checkpointer.__load(os.path.join(d, "checkpoint.pth?matching_heuristics=True"))
    ...
    def test_custom_path_manager_handler(self):
        with tempfile.TemporaryDirectory(prefix="detectron2_test") as d:
            ...
            checkpointer.__load("detectron2_test://checkpoint.pth")
            checkpointer.__load("detectron2_test://checkpoint.pth?matching_heuristics=True")
