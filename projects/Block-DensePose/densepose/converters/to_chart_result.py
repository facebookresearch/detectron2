...
class ToChartResultConverterWithBlock(BaseConverter):
    """
    Converts various DensePose predictor outputs to DensePose results.
    Each DensePose predictor output type has to register its convertion strategy.
    """
    ...
    def convert(cls, predictor_outputs: Any, boxes: Boxes, *args, **kwargs) -> DensePoseChartResult:
        ...
        Return:
            An instance of DensePoseResult. If no suitable converter was found, raises KeyError
        """
        return super(ToChartResultConverterWithBlock, cls).convert(predictor_outputs, boxes, *args, **kwargs)


class ToChartResultConverterWithDis(BaseConverter):
    registry = {}
    dst_type = DensePoseChartResult

    @classmethod
    def convert(cls, predictor_outputs: Any, boxes: Boxes, *args, **kwargs) -> DensePoseChartResult:
        return super(ToChartResultConverterWithDis, cls).convert(predictor_outputs, boxes, *args, **kwargs)
