...
class ConvFCHead(nn.Module):
    ...
    @configurable
    def __init__(self, input_shape: ShapeSpec, *, conv_dim, fc_dims, output_shape):
        """
        NOTE: this interface is experimental.
        The following attributes are parsed from config:
            conv_dim: the output dimension of the conv layers
            fc_dims: a list of N>0 integers representing the output dimensions
                of N FC layers
...
class PointRendMaskHead(nn.Module):
    ...
    def _forward_mask_coarse_loss(self, mask_coarse_logits, instances):
        return {"loss_mask": mask_rcnn_loss(mask_coarse_logits, instances)}

    def _forward_mask_point(self, features, mask_representations, instances):
        """
        Forward logic of the mask point head.
        """
        if not self.mask_point_on:
            return {} if self.training else mask_representations
        ...
        if self.training:
            proposal_boxes = [x.proposal_boxes for x in instances]
            # sample point_coords and its corresponding point_labels
            point_coords, point_labels = self._sample_train_points(mask_representations, instances)
            # sample image-level features
            fine_grained_features, _ = point_sample_fine_grained_features(
                mask_features_list, features_scales, proposal_boxes, point_coords
            )
            # predict point-wise mask logits
            point_logits = self._get_point_logits(
                fine_grained_features, point_coords, mask_representations
            )
            # point-wise loss
            return {"loss_mask_point": roi_mask_point_loss(point_logits, instances, point_labels)}
        else:
            pred_boxes = [x.pred_boxes for x in instances]
    ...
    def _mask_representation_loss(self, mask_representations, instances):
        return {"loss_mask": mask_rcnn_loss(mask_representations, instances)}

    def _sample_train_points(self, mask_representations, instances):
