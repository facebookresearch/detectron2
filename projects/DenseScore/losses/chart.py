...
class DensePoseChartLoss:
    ...
    * cross entropy loss for fine (I) and coarse (S) segmentations
    ...
    def __init__(self, cfg: CfgNode):
        ...
        self.scoring_on   = cfg.MODEL.SCORING_ON
        if self.scoring_on:
            self.dense_score_helper = DenseScoreLossHelper()
    ...
    def __call__(
        ...
        losses_uv, gps_gt, bbox_indices = self.produce_densepose_losses_uv(
            proposals_with_gt,
        ...
        losses_segm, iou_gt, iou_indices = self.produce_densepose_losses_segm(
            proposals_with_gt,
            densepose_predictor_outputs,
            packed_annotations,
            interpolator,
        ...
        return {**losses_uv, **losses_segm}, gps_gt, bbox_indices, iou_gt, iou_indices
    ...
    def produce_densepose_losses_uv(
        ...
        """
        Score part: get gps target
        """
        if not self.scoring_on:
            return loss, None, None
        else:
            i_coarse_est = interpolator.extract_at_points(
            ...
            i_coarse_est = torch.argmax(i_coarse_est, dim=1)

            i_fine_est = interpolator.extract_at_points(
            ...
            i_fine_est = torch.argmax(i_fine_est, dim=1)

            i_est = i_fine_est * (i_coarse_est > 0)
            ...
            ogps_values = self.dense_score_helper.computeOgps(
                u_gt, v_gt, i_gt, u_est, v_est, i_est
            )
            ogps_values = torch.as_tensor(ogps_values, dtype=torch.float, device=u_gt[0].device)

            point_bbox_indices = packed_annotations.point_bbox_indices[j_valid_fg]
            # bbox_indices = list(set(point_bbox_indices))
            
            scoring_gps = []
            indices = []

            for ids in packed_annotations.bbox_indices:
                gps = ogps_values[point_bbox_indices == ids].mean()
                if torch.isfinite(gps):
                    scoring_gps.append(gps)
                    indices.append(ids)

            gps_gt = [torch.as_tensor(scoring_gps, dtype=torch.float, device=u_gt[0].device)[:, None]]

            return loss, gps_gt, torch.as_tensor(indices, dtype=torch.long, device=u_gt[0].device).long()
    ...
    def produce_densepose_losses_segm(
        ...
        if not self.scoring_on:
            ...
            iou_gt = self.dense_score_helper.getmIOU(proposals_with_gt, densepose_predictor_outputs, packed_annotations)

            iou_gt = [torch.as_tensor(iou_gt, dtype=torch.float, device=fine_segm_gt[0].device)[:, None]]

            return loss, iou_gt, packed_annotations.bbox_indices
