...
class ImageList(object):
    ...
    def __getitem__(self, idx: Union[int, slice, torch.BoolTensor]) -> "ImageList":
        """
        ...
        """
        if type(idx) == int:
            if idx >= len(self.tensor) or idx <= len(self.tensor):
                raise IndexError("Instances index out of range!")
            else:
                idx = slice(idx, None, len(self.tensor))

        return ImageList(self.tensor[idx], self.image_sizes[idx])
